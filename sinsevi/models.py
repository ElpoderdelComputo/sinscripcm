...
class Capcurs(models.Model):
    id = models.BigAutoField(primary_key=True)
    cve_program = models.CharField(max_length=3)
    id_catacurs = models.ForeignKey('Catacurs', on_delete=models.CASCADE, db_column='id_catacurs', to_field='id')
    cve_curso = models.CharField(max_length=6)
    nombre = models.CharField(max_length=120)
    cve_academic = models.ForeignKey('Academic', on_delete=models.CASCADE, db_column='cve_academic',
                                     to_field='cve_academic')
...
class EstudianManager(BaseUserManager):
    def create_user(self, cve_estud, nombres, apellidos, cve_campus, cve_institu, cve_program, periingr, fechingr,
                    cve_sexo, e_mail, usename, password):
    ...
    def create_superuser(self, cve_estud, nombres, apellidos, cve_campus, cve_institu, cve_program, periingr, fechingr,
                         cve_sexo, e_mail, usename, password):
...
class Estudian(AbstractBaseUser):
    ...
    REQUIRED_FIELDS = ['cve_estud', 'nombres', 'apellidos', 'cve_campus', 'cve_institu,', 'cve_program', 'usename',
                       'password']
    ...
    def incrementar_email_ayb(self):
            self.email_ayb += 1
            self.save()
